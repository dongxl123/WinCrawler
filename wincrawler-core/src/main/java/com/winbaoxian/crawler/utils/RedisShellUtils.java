...
import com.winbaoxian.crawler.model.core.RedisCommandDTO;
...
public enum RedisShellUtils {
    ...
    private static final String SPECIAL_CHAR_STRING = WinCrawlerConstant.CHAR_DOUBLE_QUOTA + WinCrawlerConstant.CHAR_SINGLE_QUOTA;
    ...
    public RedisCommandDTO parseShellCommand(String command) {
        if (StringUtils.isBlank(command)) {
            throw new WinCrawlerException("redis command can not be null");
        }
        String[] parts = parseParts(command);
        if (ArrayUtils.isEmpty(parts)) {
            throw new WinCrawlerException(String.format("redis command is error: %s", command));
        }
        ...
        if (commandType == null || RedisSupportCommandType.RedisReplyType.NOT_SUPPORTED.equals(commandType.getReplyType())) {
            throw new WinCrawlerException(String.format("redis command(%s) is not supported: %s", typeStr, command));
        }
    ...
    private String[] parseParts(String str) {
        ...
        String separator = WinCrawlerConstant.CHAR_SPACE;
        //start = -1 表明未找到起始位置
        int start = -1;
        //标识特殊符号-单引号、双引号
    ...
    private boolean isSpecialChar(String str, int index) {
        ...
        if (SPECIAL_CHAR_STRING.indexOf(c) != -1) {
            ...
            if (index > 0) {
                ...
                if (WinCrawlerConstant.CHAR_RIGHT_SLASH.equals(String.valueOf(perChar))) {
                    return false;
                }
            }
            return true;
