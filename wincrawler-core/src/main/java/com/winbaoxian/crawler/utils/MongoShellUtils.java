...
import com.winbaoxian.crawler.enums.MongoSupportCommandType;
...
public enum MongoShellUtils {
    ...
    public MongoCommandDTO parseShellCommand(String command) {
        ...
        if (StringUtils.isBlank(tempCommand)) {
            throw new WinCrawlerException("mongo command can not be null");
        }
        tempCommand = StringUtils.trim(tempCommand);
        int idx = tempCommand.indexOf(WinCrawlerConstant.CHAR_DOT);
        MongoCommandDTO commandDTO = new MongoCommandDTO();
        if (idx == -1) {
            throw new WinCrawlerException(String.format("mongo command is error: %s", command));
        } else {
            String prefix = tempCommand.substring(0, idx);
            if (!StringUtils.equalsIgnoreCase(prefix, MONGO_COMMAND_PREFIX)) {
                throw new WinCrawlerException(String.format("mongo command prefix is error: %s", command));
            }
            commandDTO.setPrefix(prefix);
            tempCommand = tempCommand.substring(idx + 1);
        }
        idx = tempCommand.indexOf(WinCrawlerConstant.CHAR_DOT);
        if (idx == -1) {
            throw new WinCrawlerException(String.format("mongo command is error: %s", command));
        } else {
        ...
        while (matcher.find()) {
            ...
            if (count >= 2) {
                ...
                if (commandType == null) {
                    throw new WinCrawlerException(String.format("mongo command does not support this operation(%s): %s", functionStr, command));
                }
