...
public enum HttpUtils {
    ...
    public byte[] doPost(String url, String cookies) throws Exception {
        return doPost(url, cookies, null, null, null);
    }

    public byte[] doPost(String url, String cookies, String charSet, Integer timeOutSec, String contentType) throws Exception {
        ...
        RequestConfig defaultRequestConfig = RequestConfig.custom().setSocketTimeout(timeOutSec.intValue() * 1000)
                .setConnectTimeout(timeOutSec.intValue() * 1000).setConnectionRequestTimeout(timeOutSec.intValue() * 1000).build();
        CloseableHttpClient httpClient = HttpClients.custom().setDefaultRequestConfig(defaultRequestConfig).build();
        HttpPost httpPost = new HttpPost(url);
        if (StringUtils.isNotBlank(cookies)) {
            httpPost.addHeader(new BasicHeader("Cookie", cookies));
        }
        httpPost.addHeader(new BasicHeader("Content-Type", String.format("%s;charset=%s", contentType, charSet)));
    ...
    public byte[] doFileGet(String url, String cookies) {
        return doFileGet(url, null, cookies);
    }

    public byte[] doFileGet(String url, Integer timeOutSec, String cookies) {
        if (timeOutSec == null) {
            timeOutSec = Integer.valueOf(10);
        }
        byte[] result = null;
